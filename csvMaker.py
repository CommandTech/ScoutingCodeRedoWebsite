...
def convert_excel_to_csv(file_path):
    ...
    output_dir = os.path.join(script_dir, 'frontend',
                              'public', 'ExcelCSVFiles')
    # Get the code value from the config file and strip the comment
    code = config['CSVmaker']['code'].split(';')[0].strip()
    ...
    if file_name.endswith('.xlsx') or file_name.endswith('.xls'):
        ...
        with pd.ExcelFile(file_path) as xls:
            ...
            if 'Code' not in xls.sheet_names:
                print(
                    f'Error: Sheet "Code" does not exist in file {file_name}. Marking for removal.')
                remove_file = True
            else:
                ...
                if df_code.empty:
                    print(
                        f'Error: Sheet "Code" in file {file_name} is empty. Marking for removal.')
                    remove_file = True
                # Check if the value in cell A2 matches the code from the config
                elif df_code.iloc[0, 0] != code:
                    print(
                        f'Error: Cell A2 in sheet "Code" of file {file_name} does not match the code value. Marking for removal.')
                    remove_file = True
