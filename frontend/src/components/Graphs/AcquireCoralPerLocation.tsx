      ...
      if (selectedTeam) {
        try {
          ...
          const pointsColumnData = [
            { name: 'Station', value: parseFloat(((aggregatedData['AcqCoralS'] - aggregatedDataAuto['AcqCoralS']) / matchCount).toFixed(2)) },
            { name: 'Floor', value: parseFloat(((aggregatedData['AcqCoralF'] - aggregatedDataAuto['AcqCoralF']) / matchCount).toFixed(2)) }
          ];
